...
def load_raw_dataset(dataset_cfg, cache_dir: str) -> DatasetDict:
    ...
    if name in {"wikitext-2", "wikitext2", "wikitext"}:
        raw = load_dataset("Salesforce/wikitext", "wikitext-2-raw-v1", cache_dir=cache_dir)
    elif name in {"ptb_text_only", "ptb"}:
        # Using Salesforce/wikitext as alternative since ptb-text-only/ptb_text_only uses deprecated dataset scripts
        raw = load_dataset("Salesforce/wikitext", "wikitext-2-raw-v1", cache_dir=cache_dir)
    else:
